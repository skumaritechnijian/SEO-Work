...
function authHeader() { return `Basic ${Buffer.from(`${WP_USER}:${WP_PASSWORD}`).toString('base64')}`; }
...
async function uploadMedia(post) {
  ...
  await wpFetch(`/wp-json/wp/v2/media/${media.id}`, {
    ...
    body: JSON.stringify({ alt_text: `${post.title} featured image`, title: post.title, caption: '', description: `Generated square featured image for ${post.title}` })
  });
  return media.id;
}
...
function member(name, value) { return `<member><name>${escapeXml(name)}</name><value><string>${escapeXml(value)}</string></value></member>`; }
function customField(key, value) { return `<value><struct>${member('key', key)}${member('value', value)}</struct></value>`; }
...
async function main() {
  ...
  for (const post of posts) {
    ...
    manifest.push({ key: post.key, id: saved.id, title: post.title, date: post.date, status: saved.status, slug: saved.slug, link: saved.link, mediaId, categoryId: post.categoryId, tagCount: post.tagIds.length });
    console.log(`Scheduled: ${post.title} -> ${saved.link}`);
  }
  if (!DRY_RUN) fs.writeFileSync(path.join(ROOT, 'next-two-weeks-publish-result.json'), `${JSON.stringify(manifest, null, 2)}\n`, 'utf8');
}
