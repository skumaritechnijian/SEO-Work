...
const posts = JSON.parse(fs.readFileSync(path.join(ROOT, 'next-two-weeks-posts.json'), 'utf8'))
  .filter((post) => /^2026-06-0[1-5]/.test(post.key));
...
async function main() {
  ...
  for (const post of posts) {
    ...
    console.log(`Updated June long-form content: ${saved.slug} (${words} words, ${saved.status})`);
  }
  fs.writeFileSync(path.join(ROOT, 'june1-5-longform-wordpress-update.json'), `${JSON.stringify(result, null, 2)}\n`, 'utf8');
}
